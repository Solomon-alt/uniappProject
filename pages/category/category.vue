<template>
	...
	<view>
		...
		<view class="box flex">
			...
			<!-- #endif -->
			<u-search @focus="show = true" class="sea" placeholder="零食" v-model="keyword" :show-action="false">
			</u-search>
		</view>
		<!-- 下方分类 -->
		<view class="card">
			...
			<view class="tex">
				<view class="te-cer" v-for="(item,index) in arr" :key="index">
					<view class="nav" style="background-color: #F8F8F8;" :class="{'active':flag===index}"
						@click="Flag(index)">
						{{item.title}}
					</view>
				</view>
